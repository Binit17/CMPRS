    ...
    public:
        ...
        void assign_encoding(symbol_id* node, std::string encoding)
        {
            ...
            if(!node->left && !node->right)
            {
                ...
                initial_list.at(static_cast<size_t>(node->sym[0])).bit_encoding = encoding;
       ...
       //Read file serially and change to proper encoding
        ...
        void save_encoded_string(std::string_view encoded_string, std::string file_name)
        {
           ...
           for(auto i: list.at(0).sym)
           {
                ...
                encoding_string_to_bits += static_cast<uint8_t>((initial_list.at(static_cast<size_t>(i)).bit_encoding.length()));
                convert_and_store_in_bits(initial_list.at(static_cast<size_t>(i)).bit_encoding, encoding_string_to_bits);
           }
...
int main(int argc, char* argv[])
{
    ...
    std::string input_text = static_cast<std::string>(buffer);
    input_file.close();
    Symbol_manager huff;
